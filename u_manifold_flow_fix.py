...
@modulus.sym.main(config_path="conf", config_name="conf_u_flow_fix")
def run(cfg: ModulusConfig) -> None:
    ...
    no_slip = PointwiseBoundaryConstraint(
        ...
        criteria=Not(
            Or(
                geo.on_boundary_top,
                geo.on_boundary_symmetry_xz,
                geo.on_boundary_symmetry_yz,
            )
        ),
        batch_per_epoch=cfg.custom.batch_per_epoch,
    )
    ...
    inlet_sample = geo.fluid.sample_boundary(
        1000, parameterization=geo.pr, criteria=geo.on_boundary_inlet
    )
    outlet_sample = geo.fluid.sample_boundary(
        1000, parameterization=geo.pr, criteria=geo.on_boundary_outlet
    )
